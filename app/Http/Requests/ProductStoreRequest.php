...
class ProductStoreRequest extends FailedValidation
{
    ...
    public function rules()
    {
        $rule = [
            ...
            'details_one' => 'required|string|max:500',
            'price' => 'required',
            'status' => 'required',
            'serving' => 'required|numeric|min:1',
            'flavor_ids' => 'required|array|min:1',
            'flavor_ids.*' => 'required|exists:flavors,id',
        ];

        if ($this->has("details_two") and $this->details_two) {
            $rule['details_two'] = 'required|string|max:500';
        }
        if ($this->has("details_three") and $this->details_three) {
            $rule['details_three'] = 'required|string|max:500';
        }
