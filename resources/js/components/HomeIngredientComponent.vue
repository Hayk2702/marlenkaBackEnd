            ...
            <div>
                <b-row class="justify-content-end">
                    <b-col sm="5" md="5" class="d-flex table_navigate justify-content-end pb-1">
                        <div class="col-md-1">
                            <a class="btn btnAdd" href="javascript:void(0)" @click="showAddOrEditForm()"> Add Ingredient </a>
                        </div>
                    </b-col>
                </b-row>
<!--                <b-row class="justify-content-end">
                ...
                <div class="parent_card_pagination ingridients_table">
                    <div class="card">
                        <div>
                            <b-table
                                    ...
                                    :apiUrl="getRout('home_ingredient.index')"
                    ...
                    <PaginationComponent ref="PaginationComponent" v-if="Object.keys(homeIngredientData).length > 0"
                                     :paginate="paginate"
                        ...
                        <b-row class="mb-1 bRowPosition">
                            <b-col class='blockInput title_hov_top' v-b-tooltip.hover data-title="description">
                                <b-form-input
                                        ...
                                        v-model="homeIngredient.description"
                                        type="text"
                        ...
                        <b-row class="mb-1 images">
                            <b-col v-b-tooltip.hover data-title="image"
                                ...
                                <label for="image_path">
                                    <template >
                                        <img v-if="homeIngredient && homeIngredient.image_path && homeIngredient.image_path_src"
                                             :src="homeIngredient.image_path_src"
                                             alt="image_path"
                                             style="width: 100px;height:100px; margin-left: 10px;"
                                        />
                                        <img v-else-if="homeIngredient && homeIngredient.image_path_src"
                                             :src="`${homeIngredient.image_path_src}`"
                                             alt="image_path"
                                             style="width: 100px;height:100px; margin-left: 10px;"
                                        />
                    ...
                    <template #modal-footer>
                        <div class="w-100 handle_button">
                            <b-button
                                    ...
                                    @click="sendData(homeIngredient_id)"
                            >
                               Edit
                            </b-button>
                        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            let self = this;
            PageName.$emit('updatePageName', "Home Ingredients");
        ...
        methods: {
            ...
            handleFileUploadImage(event) {
                ...
                if (event.target && event.target.files && event.target.files[0]) {
                    self.homeIngredient.image_path = event.target.files[0];
                    self.homeIngredient.image_path_src = URL.createObjectURL(event.target.files[0]);
                }
            },
            ...
            getData(data) {
                let self = this;
                return axios.get(route('home_ingredient.index') + `?page=${data.currentPage}`, {params: data})
                    .then((resp) => {
                        if (resp && resp.data && resp.data.data) {
                            self.homeIngredientData = resp.data.data;
                            ...
                            self.isBusy = false;
                            return self.homeIngredientData;
                        }
                    })
            ...
            sendData(id) {
                ...
                let self = this;
                self.errors = {};
                let data = Object.assign({}, self.homeIngredient);

                if (id) {
                    data.id = id;
                }
                const formData = new FormData();
                formData.append('image_path', self.homeIngredient.image_path);
                formData.append('image_path_src', self.homeIngredient.image_path_src);
                for (let key in data) {
                    formData.append(key, data[key]);
                }
                axios.post(route('home_ingredient.store'), formData)
                    .then((data) => {
            ...
            showAddOrEditForm(obj = '', addOrEdit = 'add') {
                ...
                self.homeIngredient_id = obj ? obj.id : "";
                self.homeIngredient.title = obj ? obj.title : "";
                self.homeIngredient.description = obj ? obj.description : "";
                self.homeIngredient.image_path = "";
                self.homeIngredient.image_path_src = obj ? obj.image_path : "";
                self.show = true;
            },
            ...
            destroyItem(id) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(route('home_ingredient.destroy', {home_ingredient: id}))
                            .then((data) => {
                                if (data && data.data && data.data.isSuccess) {
