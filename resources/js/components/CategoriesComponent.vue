        ...
        <div class="row justify-content-center " style="margin-top: 50px; ">
            <div>
                ...
                <div class="parent_card_pagination">
                    <div class="card">
                        <div>
                            <b-table
                                    ...
                                    :apiUrl="getRout('categories.index')"
                    ...
                    <PaginationComponent ref="PaginationComponent" v-if="Object.keys(categories).length > 0"
                                     :paginate="paginate"
                ...
                >
                    ...
                    <b-container fluid class="slider_modal">
                        <b-row class="mb-1 bRowPosition">
                            <b-col class='blockInput title_hov_top' v-b-tooltip.hover data-title="name">
                                <b-form-input
                        ...
                        <b-row class="mb-1 images">
                            <b-col v-b-tooltip.hover data-title="image"
                                ...
                                <label for="image_path">
                                    <template >
                                        <img v-if="category && category.image_path && category.image_path_src"
                                             :src="category.image_path_src"
                                             alt="image_path"
                                             style="width: 100px;height:100px; margin-left: 10px;"
                                        />
                                        <img v-else-if="category && category.image_path_src"
                                             :src="`${category.image_path_src}`"
                                             alt="image_path"
                                             style="width: 100px;height:100px; margin-left: 10px;"
                                        />
...
<script>
    ...
    export default {
        ...
        mounted() {
            let self = this;
            PageName.$emit('updatePageName', "Categories");
        ...
        methods: {
            ...
            handleFileUploadImage(event) {
                ...
                if (event.target && event.target.files && event.target.files[0]) {
                    self.category.image_path = event.target.files[0];
                    self.category.image_path_src = URL.createObjectURL(event.target.files[0]);
                }
            },
            ...
            getData(data) {
                let self = this;
                return axios.get(route('categories.index') + `?page=${data.currentPage}`, {params: data})
                    .then((resp) => {
            ...
            sendData(id) {
                ...
                let self = this;
                self.errors = {};
                let data = Object.assign({}, self.category);

                if (id) {
                    data.id = id;
                }
                const formData = new FormData();
                formData.append('image_path', self.category.image_path);
                formData.append('image_path_src', self.category.image_path_src);
                for (let key in data) {
            ...
            showAddOrEditForm(obj = '', addOrEdit = 'add') {
                ...
                self.category_id = obj ? obj.id : "";
                self.category.name = obj ? obj.name : "";
                self.category.description = obj ? obj.description : "";
                self.category.image_path = "";
                self.category.image_path_src = obj ? obj.image_path : "";
