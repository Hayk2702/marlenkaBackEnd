...
<!--                <b-row class="justify-content-end">
                ...
                <div class="parent_card_pagination">
                    ...
                    <PaginationComponent ref="PaginationComponent" v-if="Object.keys(faqs).length > 0"
                                     :paginate="paginate"
                ...
                >
                    ...
                    <b-container fluid class="slider_modal">
                        <b-row class="mb-1 bRowPosition">
                            <b-col class='blockInput title_hov_top' v-b-tooltip.hover data-title="question">
                                <b-form-input
                                        :class="[errors['question'] ? 'error-border': '', 'addFormInputs']"
                                        v-model="faq.question"
                                        type="text"
                                        placeholder="question"
                                ></b-form-input>
                                <small v-if="errors['question']" class="error-msg">{{errors['question'] }}</small>
                            </b-col>
                        </b-row>
                        <b-row class="mb-1 bRowPosition">
                            <b-col class='blockInput title_hov_top' v-b-tooltip.hover data-title="answer">

                                <ckeditor :editor="editor" v-model="faq.answer" :class="[errors['answer'] ? 'error-border': '', 'addFormInputs']"></ckeditor>

                                <small v-if="errors['answer']" class="error-msg">{{errors['answer'] }}</small>
                            </b-col>
                        </b-row>
                    </b-container>
...
<script>
    ...
    export default {
        ...
        mounted() {
            let self = this;
            PageName.$emit('updatePageName', "FAQ");
        ...
        methods: {
            ...
            getData(data) {
                let self = this;
                return axios.get(route('faqs.index') + `?page=${data.currentPage}`, {params: data})
                    .then((resp) => {
                        if (resp && resp.data && resp.data.data) {
                            self.faqs = resp.data.data;
            ...
            sendData(id) {
                ...
                let self = this;
                self.errors = {};
                let data = Object.assign({}, self.faq);
                ...
                axios.post(route('faqs.store'), formData)
                    .then((data) => {
            ...
            showAddOrEditForm(obj = '', addOrEdit = 'add') {
                ...
                self.faq_id = obj ? obj.id : "";
                self.faq.question = obj ? obj.question : "";
                self.faq.answer = obj ? obj.answer : "";
            ...
            destroyItem(id) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(route('faqs.destroy', {faq: id}))
                            .then((data) => {
                                if (data && data.data && data.data.isSuccess) {
